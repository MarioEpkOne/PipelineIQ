...
6. **Wire dependencies** -- determine which child specs depend on which others. For each dependency edge, intentionally embed 2+ title keywords (> 4 chars) from the dependency's title into the dependent's Goal or Current State text as whole words (not inside compound identifiers). Keywords must survive word-boundary matching: they should appear as standalone words or hyphen-separated segments, not embedded in underscored compounds or longer tokens. **[CONTRACT: keyword-stopwords]** Stopword list (15 words), threshold (2+), and word-boundary matching are shared with pipeline-team.md — changing one without the other breaks dependency detection.
