...
| **Scope ↔ Steps reconciliation** | Every file in the Scope list must be the `**File**:` target of at least one numbered Step, or be explicitly annotated "read-only context." A test file that appears in Scope but is written by no step is a silent coverage gap (e.g. a spec mandating four assertions whose plan listed the test file but never added a step to write them — shipped with zero coverage until the auditor caught it). Convert each spec Testing-Strategy assertion into a step that names the file and the exact assertions. |
